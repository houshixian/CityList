...
static const CGFloat STTextFieldHeight = 30;
static const CGFloat STSearchBarMargin = 15;
static const CGFloat STSearchBarYMargin = (STSearchBarHeight - STTextFieldHeight)/2;
static const CGFloat STSearchbuttonCancelW = 59;
...
- (void)setupUI{
    ...
    _placeholderColor = BRCOLOR(@"#AAAAAA");
...
- (UIView *)BottomLineView{
    ...
    if (!_BottomLineView) {
        _BottomLineView = [[UIView alloc]initWithFrame:CGRectMake(0, self.height - 0.5, WIDTH, 0.5)];
        _BottomLineView.backgroundColor = BRCOLOR(@"#e5e5e5");
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        if (self.showsCancelButton) {
            self.buttonCancel.frame = CGRectMake(self.frame.size.width - STSearchbuttonCancelW, 0, STSearchbuttonCancelW, STSearchBarHeight);
            self.textField.frame = CGRectMake(STSearchBarMargin, STSearchBarYMargin, self.buttonCancel.frame.origin.x-STSearchBarMargin, STTextFieldHeight);
        }
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        if (self.showsCancelButton) {
            self.buttonCancel.frame = CGRectMake(self.frame.size.width, 0, STSearchbuttonCancelW, STSearchBarHeight);
            self.textField.frame = CGRectMake(STSearchBarMargin, STSearchBarYMargin, self.frame.size.width-STSearchBarMargin*2, STTextFieldHeight);
        }
...
{
    if (!_textField) {
        UITextField *textField = [[UITextField alloc] initWithFrame:CGRectMake(STSearchBarMargin, STSearchBarYMargin, self.frame.size.width-STSearchBarMargin*2, STTextFieldHeight)];
...
//        textField.layer.borderWidth= 0.5f;
        textField.backgroundColor = [UIColor colorWithRed:(238.0/255) green:(238.0/255) blue:(238.0/255) alpha:1];
...
- (UIButton *)buttonCancel
{
    if (!_buttonCancel) {
        ...
        buttonCancel.frame = CGRectMake(self.frame.size.width, 0, STSearchbuttonCancelW, STSearchBarHeight);
        buttonCancel.titleLabel.font = [UIFont systemFontOfSize:15.0f];
        [buttonCancel addTarget:self
                         action:@selector(cancelButtonTouched)
               forControlEvents:UIControlEventTouchUpInside];
        [buttonCancel setTitleColor:BRCOLOR(@"#333333") forState:UIControlStateNormal];
...
//        buttonCancel.contentHorizontalAlignment = UIControlContentHorizontalAlignmentRight;
...
//        buttonCenter.titleEdgeInsets = UIEdgeInsetsMake(0, 0, 0, -10);
        
        [buttonCenter.titleLabel setFont:[UIFont systemFontOfSize:15]];
